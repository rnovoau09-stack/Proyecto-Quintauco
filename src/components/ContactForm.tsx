...
const ContactForm = ({ className = "", onSuccess }: ContactFormProps) => {
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    const tipo_consulta =
      formData.get("tipo_consulta") === "inmobiliaria"
        ? "inmobiliaria"
        : "parque";
    ...
    try {
      ...
      const [emailResult, dbResult] = await Promise.allSettled([
        ...
        supabase
          .from("solicitudes")
          .insert({ name, email, phone, message, tipo_consulta }),
      ]);
      ...
      if (!emailSuccess && !dbSuccess) {
        ...
        toast.warning(
          t("contact.form.success") +
            " (El correo no pudo enviarse, pero tu mensaje fue guardado)",
        );
      } else if (!dbSuccess) {
        // Solo falló la base de datos
        toast.warning(
          t("contact.form.success") + " (El correo fue enviado correctamente)",
        );
      } else {
      ...
      <div className="space-y-2">
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="inmobiliaria">
              {t("contact.form.typeRealEstate")}
            </Label>
          </div>
          <div className="flex items-center space-x-2">
