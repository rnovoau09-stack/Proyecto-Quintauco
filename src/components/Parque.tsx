...
import { Trees, Mountain, Map, Footprints, Bike, Tent, Leaf, Binoculars, Bird, Camera, Sun, Home as HomeIcon, Activity, Expand } from "lucide-react";
...
const Parque = () => {
  ...
  return (
    <section id="etapas" className="py-12 md:py-16 sm:py-24 md:py-32 bg-gradient-to-b from-background via-background to-muted/20 relative overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]">
      {/* Modern Decorative Background Elements - Hidden on mobile */}
      <div
        className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent hidden md:block"
      />
      <div
        className="absolute bottom-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent hidden md:block"
      />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <FadeIn className="max-w-4xl mx-auto text-center mb-8 md:mb-12 sm:mb-16 md:mb-24">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-foreground dark:text-card-foreground mb-4 md:mb-6 sm:mb-8 leading-tight px-2">
            <span className="gradient-text text-shadow-premium">{t('park.header.title')}</span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg text-muted-foreground dark:text-muted-foreground leading-relaxed max-w-3xl mx-auto px-2">
            {t('park.header.descStart')} <strong className="text-primary font-semibold">{t('park.header.descHighlight')}</strong> {t('park.header.descEnd')}
          </p>
        </FadeIn>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 sm:gap-8 mb-12 md:mb-16 sm:mb-24 md:mb-32">
          {stats.map((stat, index) => (
            <ScaleIn key={index} delay={index * 0.08} duration={0.55} className="relative group">
              ...
              <div
                className="relative bg-card backdrop-blur-xl border-2 border-border/70 rounded-xl md:rounded-2xl sm:rounded-3xl p-4 md:p-6 sm:p-8 md:p-10 text-center hover:shadow-premium-lg hover:border-primary/50 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] shadow-premium"
              >
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-2 gap-6 md:gap-8 sm:gap-12 md:gap-16 lg:gap-24 items-center">
            <FadeIn className="order-2 lg:order-1 space-y-4 md:space-y-6 sm:space-y-8 md:space-y-10">
              <div className="flex items-center gap-2 md:gap-3 sm:gap-4 md:gap-5">
                ...
                <h3 className="text-xl md:text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-foreground dark:text-card-foreground">{t('park.masterplan.title')}</h3>
              </div>
              ...
              <ul className="space-y-3 md:space-y-4 sm:space-y-5 md:space-y-6">
                {[
                  { title: t('park.masterplan.base.title'), desc: t('park.masterplan.base.desc'), icon: Tent },
                  { title: t('park.masterplan.residential.title'), desc: t('park.masterplan.residential.desc'), icon: HomeIcon },
                  { title: t('park.masterplan.sports.title'), desc: t('park.masterplan.sports.desc'), icon: Bike }
                ].map((item, idx) => (
                  <li key={idx} className="flex gap-2 md:gap-3 sm:gap-4 md:gap-5 group/item">
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-sm md:text-base sm:text-lg text-foreground dark:text-card-foreground mb-1">{item.title}</h4>
                      <p className="text-xs md:text-sm sm:text-base text-muted-foreground dark:text-muted-foreground leading-relaxed">{item.desc}</p>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="order-1 lg:order-2 relative group w-full">
              ...
              <Link
                to="/plano-detalle"
                state={{ scrollPosition: window.scrollY, returnTo: "/", sectionId: "etapas" }}
          ...
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 md:gap-16 lg:gap-24 items-center">
            ...
            <FadeIn className="space-y-6 sm:space-y-8 md:space-y-10">
              <div className="flex items-center gap-3 sm:gap-4 md:gap-5">
                ...
                <h3 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-foreground dark:text-card-foreground">{t('park.conservation.title')}</h3>
              </div>
              <p className="text-base sm:text-lg md:text-xl text-muted-foreground dark:text-muted-foreground leading-relaxed">
                {t('park.conservation.desc')} <strong className="text-accent font-semibold">{t('park.conservation.descHighlight')}</strong> {t('park.conservation.descEnd')}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {[
                  ...
                  <div key={idx} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-br from-accent/10 to-accent/5 border border-accent/20 hover:border-accent/40 hover:shadow-lg transition-all duration-350 ease-[cubic-bezier(0.16,1,0.3,1)] group/item">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-accent/20 flex items-center justify-center group-hover/item:scale-110 group-hover/item:rotate-3 transition-all duration-350 ease-[cubic-bezier(0.16,1,0.3,1)]" style={{ willChange: 'transform' }}>
                      <item.icon className="w-4 h-4 sm:w-5 sm:h-5 text-accent transition-transform duration-350 ease-[cubic-bezier(0.16,1,0.3,1)]" />
                    </div>
                    <span className="font-semibold text-sm sm:text-base text-foreground dark:text-card-foreground">{item.text}</span>
                  </div>
                ))}
              </div>
        ...
        <FadeIn className="text-center mb-12 sm:mb-16 md:mb-20">
          <h3 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-foreground dark:text-card-foreground mb-4 sm:mb-6 px-2">{t('park.experiences.title')}</h3>
          ...
          {experiences.map((exp, index) => (
            <ScaleIn key={index} delay={index * 0.08} duration={0.5}>
              <Card className="p-6 sm:p-8 hover:shadow-premium-lg transition-all duration-300 ease-out border-2 border-border/70 hover:border-primary/40 hover:-translate-y-2 hover:scale-[1.02] card-hover group relative overflow-hidden bg-card backdrop-blur-xl shadow-premium">
                ...
                <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl ${exp.bg} flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] shadow-lg`}>
                  <exp.icon className={`w-7 h-7 sm:w-8 sm:h-8 ${exp.color} transition-transform duration-400 ease-[cubic-bezier(0.16,1,0.3,1)]`} />
                </div>
                <h4 className="font-bold text-lg sm:text-xl text-foreground dark:text-card-foreground mb-2 sm:mb-3 relative z-10">{exp.title}</h4>
                <p className="text-xs sm:text-sm text-muted-foreground dark:text-muted-foreground leading-relaxed relative z-10">{exp.description}</p>
              </Card>
            </ScaleIn>
          ))}
        </div>
